...
if (isset($_POST['confirm_cancel']) && $_POST['confirm_cancel'] === 'yes' && isset($_POST['id'])) {
    $event_id = $_POST['id'];

    $stmt = $conn->prepare("UPDATE EVENTS SET Status = 2 WHERE ID = ?");
    $stmt->bind_param("i", $event_id);
    $stmt->execute();

    if ($stmt->affected_rows > 0) {
        $_SESSION['success_msg'] = "Event canceled successfully.";
    } else {
        $_SESSION['error_msg'] = "Error canceling event.";
    }
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <div class="delb">
                        <div class="row">
                            <div class="col-4">
                                <button type="submit" name="confirm_cancel" value="no" class="btn btn-primary btn-block">No</button>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-4">
                                <button type="submit" name="confirm_cancel" value="yes" class="btn btn-primary btn-block">Yes</button>
                            </div>
                        </div>
                    </div>
                </form>
