...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['field'])) {
    ...
    if ($field === 'password') {
        if (isset($_POST['old_password'], $_POST['new_password'], $_POST['confirm_password'])) {
            ...
            $stmt->bind_param("i", $_SESSION['editing']);
            $stmt->execute();
            ...
            if (!password_verify($old_password, $current_password)) {
                ...
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("si", $hashed_password, $_SESSION['editing']);
    ...
    } elseif ($field === 'position' && isset($_POST['new_value'])) {
        $new_position = $_POST['new_value'];
        ...
        if ($new_position < $_SESSION['position']) {
            $error_message = "You cannot assign a position higher than your own.";
        } else {
            $sql = "UPDATE ADMIN SET Position = ? WHERE UserID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ii", $new_position, $_SESSION['editing']);
        ...
        if ($error_message === '') {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $new_value, $_SESSION['editing']);
            ...
            <div class="card-body">
                ...
                <?php if ($field !== '') { ?>
                    <form action="" method="post">
                        ...
                        <?php } elseif ($field === 'position') { ?>
                            <div class="input-group mb-3">
                                <input type="number" name="new_value" class="form-control" placeholder="Enter new <?php echo htmlspecialchars($label); ?>" required>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-6">
                                <a href="admins_edit.php" class="btn btn-secondary btn-block">Cancel</a>
                            </div>
                            <div class="col-6">
